        ...
        if (has) {
            ...
            [httpManager POST:@"/share"
                   parameters:@{
                     ...
                     progress:nil
                      success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                          NSLog(@"%@(ID=%ld) share success", cell.fileInfo.name, cell.fileInfo.ID);
                     }
                      failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
- (void)selectCreate
{
    ...
    // 创建一个弹框
    UIAlertController *addAlertVC = [UIAlertController alertControllerWithTitle:@"新建文件夹" message:@"请输入文件夹名称" preferredStyle:UIAlertControllerStyleAlert];
