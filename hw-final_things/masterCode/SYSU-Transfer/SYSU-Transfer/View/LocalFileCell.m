...
- (void)loadPage
{
    ...
    if (_isFloder) {
        NSArray *arr = [FileManager ReadDirByPath:self.path];
        _file_size = [[NSString alloc] initWithFormat:@"%ld个文件", arr.count];
        _file_time = @"暂无";
    } else {
        _file_data = [FileManager ReadFileByName:self.path];
        _file_size = @"暂无";
        _file_time = @"暂无";
    }
